 ...
 * 成否、エラー分類、pseudonymize化した利用者識別子)だけを型で受け付け、
 ...
 */
export function pseudonymizeSubjectId(subjectId: string): string {
  return createHmac("sha256", Buffer.from(env.LOG_HMAC_KEY, "base64"))
...
/**
 * 1行1eventのJSONを出力する。ログ出力自体の失敗で業務処理を止めない
 * (設計 §15.2)。
 */
export function logOperationEvent(event: OperationLogEvent): void {
  try {
    const line = {
