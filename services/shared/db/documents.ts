 ...
 *
 * Web(`app/lib/db/documents.server.ts`)とDisplay(`services/display/`)の双方が
 * 同じSQL・同じZod schemaを使うため、実処理は`services/shared/db/`へ集約する
 * (docs/ARCHITECTURE.md)。このモジュールは環境変数もPoolも持たず、`executor`
 * (`Pool`またはtransaction中の`PoolClient`)を必ず引数で受け取る。Web向けの
 * 「省略時は`getPool()`」という既定値は`app/lib/db/documents.server.ts`側で足す。
 */
import { z } from "zod";
